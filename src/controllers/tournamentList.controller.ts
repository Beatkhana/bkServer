...
export class tournamentListController extends controller {
    async getAll(req: express.Request, res: express.Response) {
        let tournaments = await DatabaseService.query(`SELECT CAST(owner AS CHAR) as owner, id, name, image, date as startDate, endDate, discord, twitchLink, prize, info, archived, first, second, third FROM tournaments`);
    ...
    async getActive(req: express.Request, res: express.Response) {
        ...
        ${userRoles}
        WHERE archived = 0 AND t.is_mini = 0 ${sqlWhere}`,
    ...
    async getActiveMini(req: express.Request, res: express.Response) {
        ...
        LEFT JOIN tournament_settings ts ON ts.tournamentId = t.id  
        ${userRoles}
        WHERE archived = 0 AND t.is_mini = 1 ${sqlWhere}`,
            [user?.discordId, user?.discordId]
        );
        return res.send(tournaments);
    }
    ...
    async getArchive(req: express.Request, res: express.Response) {
        ...
        let data: any = await DatabaseService.paginationQuery(
            "tournaments",
            page,
            limit,
            "SELECT CAST(owner AS CHAR) as owner, id as tournamentId, name, image, date as startDate, endDate, discord, twitchLink, prize, info, archived, first, second, third FROM tournaments WHERE archived = 1 ORDER BY endDate DESC"
        );
