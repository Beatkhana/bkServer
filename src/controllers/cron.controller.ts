...
export class cronController extends controller {
    ...
    private async updateUsersDiscord() {
        const users: User[] = await DatabaseService.query("SELECT * FROM users");
        let updated = 0;
        for (const user of users) {
            if (user.refresh_token != null) {
                ...
                let response = await fetch("https://discord.com/api/oauth2/token", {
                    ...
                    .then((info: any) => {
                        ...
                        if (info.error) {
                            ...
                            DatabaseService.query("UPDATE users SET refresh_token = NULL WHERE discordId = ?", [user.discordId]);
                ...
                if (response?.username != null) {
                    try {
                        updated++;
                        await DatabaseService.query("UPDATE users SET name = ?, avatar = ?, refresh_token = ? WHERE discordId = ?", [response.username, response.avatar, refresh_token, user.discordId]);
                    } catch (error) {
                        console.error(error);
                        // throw error;
                    }
    ...
    private async updateUsersSS() {
        const users: User[] = await DatabaseService.query("SELECT * FROM users");
        let curBadges: badge[] = await DatabaseService.query(`SELECT * FROM badges`);
        let badgeAssignment = await DatabaseService.query<any>(`SELECT * FROM badge_assignment`);
        if (!Array.isArray(badgeAssignment)) return;
        ...
        for (let i = 0; i < users.length; i++) {
            ...
            if (ssData != null && ssData && ssData?.banned != 1) {
                ...
                try {
                    await DatabaseService.query("UPDATE users SET ? WHERE discordId = ?", [info, user.discordId]);
                    updated++;
                } catch (error) {
                    console.log(error);
                }
                if (ssData.badges.length > 0) {
                    for (const badge of ssData.badges) {
                        ...
                        if (!badgeLabels.includes(imgName)) {
                            ...
                            const webpData = await sharp(buff).resize({ width: 80, height: 30 }).png().toBuffer();

                            await sharp(webpData).toFile(savePath + imgName + ".png");
                            try {
                                let result: any = await DatabaseService.query(`INSERT INTO badges (image, description) VALUES (?)`, [[imgName, badge.description]]);
                                curBadges.push({
                                    id: result.insertId,
                                    image: imgName,
                        ...
                        if (!badgeAssignment.find(x => x.badgeId == curBadge.id)) {
                            try {
                                let result: any = await DatabaseService.query(`INSERT INTO badge_assignment (badgeId, userId) VALUES (?)`, [[curBadge.id, user.discordId]]);
